...
PowerShell을 실행하고 프로젝트 루트 폴더로 이동한 뒤 가상환경 파이썬을 가리켜 작동시킵니다.
...
### ① 테스트용 오디오 문장 변경/추가하기
[stt_evaluation.py](~/stt_evaluation.py#L274) 파일 내 `generate_tts_samples` 함수 안의 **`tts_configs`** 리스트를 수정해 주시면 됩니다. 
...
[stt_evaluation.py](~/stt_evaluation.py#L416)의 `main()` 함수 초반부에서 불러올 모델 사이즈를 직접 지정할 수 있습니다:
...
작업 중이신 [tts_test.ipynb] 내의 새 셀에 아래 코드를 그대로 붙여넣고 실행하면, 오디오 파일 없이도 임의의 두 텍스트 간 정확성을 즉석에서 계산하고 비교 채점할 수 있습니다.
